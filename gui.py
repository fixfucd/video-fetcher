...
# other entry points, not only through main().
try:
    import tkinter as tk
    from tkinter import ttk, filedialog, messagebox
except ImportError:  # reported by main() with a friendly message
    tk = ttk = filedialog = messagebox = None

# Log batching: yt-dlp can emit hundreds of lines per second. Posting one Tk
# callback per line floods the event queue and stalls redraws, so worker threads
# only append to this queue and the main thread drains it in batches.
_LOG_QUEUE = queue.SimpleQueue()
_LOG_FLUSH_MS = 80        # main-loop drain interval
_LOG_MAX_LINES = 4000     # keep the Text widget bounded on long downloads
_LOG_TAIL_SLICE = 2000    # lines discarded once the cap is exceeded
_LOG_CLEAR = "__clear__"  # queue sentinel: clear before inserting later lines
...
try:
    from fetch import (
        ...
        find_cookies_file, available_browsers_from,
        ...
        try_browser_cookies, same_tier, _print_douyin_web_detail_help,
    )
    FETCH_IMPORT_ERROR = None
except ImportError as _exc:  # main() reports this with a friendly message
    FETCH_IMPORT_ERROR = _exc
...
class VideoFetcherGUI:
    ...
    def _refresh_browser_detection(self):
        ...
        return self._installed_browsers
    ...
    # ─── log pipeline: queue from any thread, drain on the Tk main thread ───

    def _start_log_flush(self):
        self._log_flush_job = self.root.after(_LOG_FLUSH_MS, self._flush_log)

    def _flush_log(self):
        """Insert every queued line in one widget update, then reschedule."""
        batch = []
        try:
            while True:
        ...
        if batch:
            try:
                ...
                # Bound the widget: a long download would otherwise grow it
                # without limit and slow every insert down.
                try:
                    line_count = int(self.log.index("end-1c").split(".")[0])
                    if line_count > _LOG_MAX_LINES:
                        self.log.delete("1.0", f"{line_count - _LOG_TAIL_SLICE}.0")
    ...
    def _stop_log_flush(self):
        job = getattr(self, "_log_flush_job", None)
        if job is not None:
    ...
    def _build_dropdown(self, parent):
        ...
        self._dropdown_parent = parent  # remembered so refresh can reuse it
        # Refresh the value list in place. Destroying and repacking moved the
        # widget to the end of its container on every refresh.
        if getattr(self, "_browser_combo", None) is not None:
            try:
                self._browser_combo.configure(values=ik+ok)
                return
            except tk.TclError:
                pass
    ...
    def _on_refresh(self):
        ...
        self._refresh_browser_detection()
        if getattr(self, "_dropdown_parent", None) is not None:
            self._build_dropdown(self._dropdown_parent)
        self._update_browser_bar()
        self._update_cookies_status()
        self._log("Done.\n", "success")

    def _check_env(self):
        """Report tool availability without blocking window startup.

        The probes spawn subprocesses (up to 10s each), so they run only
        after the window is mapped and the main loop is turning.
        """
        try:
            self._log("Checking yt-dlp / ffmpeg / cookie backends...\n", "dim")
            if not check_tool("yt-dlp"):
                self._log("yt-dlp not found. pip install yt-dlp\n", "error")
            ...
            if _has_cdp(self._installed_browsers): self._log("CDP cookie fallback: available\n", "success")
            else: self._log("CDP cookie fallback: NOT AVAILABLE\n", "dim")
        except tk.TclError:
            pass  # window closed while probing

    def _log(self, text, tag="info"):
        """Thread-safe: queue the line, the main thread inserts it in batches."""
        _LOG_QUEUE.put((text, tag))

    def _clear_log(self):
        _LOG_QUEUE.put((None, _LOG_CLEAR))

    def _on_close(self):
        """Stop the download, then tear the window down cleanly."""
        if getattr(self, "_closing", False):
            return
        self._closing = True
        self._cancel_event.set()
        _terminate_process_tree(self.process)
        self._stop_log_flush()
        log("info", "GUI closed")
        close_log()
    ...
    def _show_cookie_result(self, cookie_file, label, method=""):
        ...
        if domains:
            # Rank by label depth without discarding the labels: sorting the
            # stripped names made every key equal, so the order was random.
            top = sorted(domains, key=lambda d: (-d.count('.'), d))[:8]
    ...
    def _worker_impl(self, url, plat, out):
        ...
        inst = self._refresh_browser_detection()
        avail = available_browsers_from(inst)
        self._log(f"Platform: {plat}\n", "info")
        ...
        if pref and inst.get(pref,{}).get("installed"):
            ...
            ok = self._try_cookie_chain(url, out, high, pref, plat)
        ...
        # alternates — reuse the detection pass made above
        alts = [b for b in available_browsers_from(inst, exclude=pref) if b not in tried]
        if alts:
            self._log(f"\nAlternates: {', '.join(BROWSER_CONFIG.get(b,{}).get('label',b) for b in alts)}\n", "info")
        for b in alts:
            tried.add(b)
            self._log(f"--- HD ({BROWSER_CONFIG.get(b,{}).get('label',b)}) ---\n", "info")
            ok = self._try_cookie_chain(url, out, high, b, plat)
            if ok: self._done(0); return
            if self._cancel_event.is_set(): self._done(1, "Stopped"); return
            if plat == "douyin" and is_douyin_web_detail_failure(self._last_ytdlp_tail):
                douyin_web_detail_seen = True
        ...
        # Same request, different label: repeating it cannot change the result.
        if same_tier(high, fallback):
            self._log("\nFallback tier is identical to the HD settings; skipping the repeat.\n", "warn")
            self._done(public_rc if public_rc is not None else 1); return
        self._log("\n--- LQ fallback ---\n", "warn")
    ...
    def _report_douyin_web_detail_failure(self):
        """Reuse fetch's douyin guidance instead of keeping a second copy."""
        buf = io.StringIO()
        try:
        ...
        text = buf.getvalue()
        if text:
            self._log(text if text.endswith("\n") else text + "\n", "warn")

    def _try_cookie_chain(self, url, out, high, bk, plat="generic"):
        """Run fetch.try_browser_cookies() for one browser, logging into the GUI.

        Delegates to the single shared implementation so the GUI and the CLI
        cannot drift apart (the GUI copy had already lost the cookie-lock
        diagnostic and a different step-2 condition).
        """
        self._last_ytdlp_tail = ""
        ok, diagnostic = try_browser_cookies(
            url, out, high, self.config, bk,
            platform=plat,
            emit=self._log,
            run=self._run_with_args,
            is_cancelled=self._cancel_event.is_set,
        )
        if diagnostic:
            # Distinguish a real yt-dlp tail (used for douyin detection) from
            # a status message such as "not logged in".
            looks_like_output = ("ERROR" in diagnostic or "error" in diagnostic.lower()
                                 or "fail" in diagnostic.lower() or "Traceback" in diagnostic)
            self._last_ytdlp_tail = diagnostic if looks_like_output else ""
            if not ok and not looks_like_output:
                self._log(f"  {diagnostic}\n", "warn")
        return ok
...
def main():
    ...
    if FETCH_IMPORT_ERROR is not None:
        print(f"import failed: {FETCH_IMPORT_ERROR}", file=sys.stderr)
        input("Press Enter..."); sys.exit(1)
    ...
    # Tool probing spawns subprocesses; run it once the window is on screen
    # instead of blocking the first paint.
    root.after_idle(app._check_env)
...
if __name__ == "__main__":
    # SystemExit must keep its code (main() returns 1 when startup fails).
    try:
        main()
    except Exception:
        _crash_log(sys.exc_info()); input("\nPress Enter..."); sys.exit(1)
